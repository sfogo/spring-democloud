...
### Run it all locally
* One option is to `cd` to each application and start them individaully with `mvn spring-boot:run`, making sure you start with `config-server` (for fail-fast reasons explained in the [overview](#overview)), then on to `eureka` and other applications.
* You can also use the [run all](#TODO) script. It does some _rustic_ waiting but in a real deployment you should rely on options provided by your envoronment (for instance a combination of Spring `fail fast` and Docker `restart always` options).
